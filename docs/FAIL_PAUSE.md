...
Before fail-pause, the download handler runs the automatic **recovery ladder**. After impersonate it **must** continue to silent Firefox when cookies have not been tried yet. That silent path is the same code as the modal buttons: Import from Firefox (`import_cookies_from_browser` + file-only validate, **60s total timeout**) → interruptible backoff → retry this job (`tried: impersonate, silent_firefox_cookies, backoff:N, retry`). Timeout or import fail opens fail-pause **once**; it must not leave the halt latch set without a modal, and Download all / Retry / Skip / Stop clear the latch. **A modal that only shows `tried: impersonate` is a regression** when Firefox cookies would have worked. Human fail-pause for **any** site appears only after that path is skipped, invalid, timed out, aborted, or the retry still fails. See [MULTI_SITE.md](MULTI_SITE.md) and [COOKIES.md](COOKIES.md).
